...
int main(int argc, char * const argv[]) {
    ...
    long long elapsed_time = (end2.tv_sec - start2.tv_sec) * 1e9 + (end2.tv_nsec - start2.tv_nsec);
    struct rusage usage;
    ...
    if (!silent && !findAlignment) {
        ...
        FILE *file = fopen("./scores/edlib-score.txt", "w");
        if (file == NULL)
        {
            perror("fopen");
            exit(EXIT_FAILURE);
        ...
        for (int i = 0; i < numQueries; i++) {
            if (scores[i] > -1 && (scoreLimit == -1 || scores[i] <= scoreLimit)) {
                ...
                fprintf(file, "%d %lld %ld\n", scores[i], elapsed_time, peak_memory_kb);
                if (numLocations[i] > 0) {
                    printf("  [");
